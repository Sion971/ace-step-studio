...
if "%CUDA_VERSION%"=="cpu" (
    ...
    REM --extra-index-url, pas --index-url : l'index cu130 ne publie pas
    REM (encore) sa propre copie de cuda-bindings==13.0.3, une dependance
    REM de torch pour cette version - confirme comme un vrai defaut connu
    REM (pytorch/pytorch#172926). --index-url seul echoue avec "no matching
    REM distribution found" ; --extra-index-url laisse uv completer via
    REM PyPI, ou cuda-bindings est correctement publie.
    uv pip install torch==%TORCH_VERSION% torchaudio==%TORCHAUDIO_VERSION% torchvision --extra-index-url https://download.pytorch.org/whl/%CUDA_VERSION%
) else (
...
REM --no-deps INDISPENSABLE sur CPU et cu130. Le pyproject.toml de
REM nano-vllm epingle en dur flash-attn sur une roue CUDA (sdbds/
REM flash-attention-for-windows, cu128, torch2.7.1) sans condition de
REM plateforme - confirme en pratique : uv l'installait meme avec
REM CUDA_VERSION=cpu (roue inutilisable sans GPU), ET avec CUDA_VERSION=
REM cu130 (roue torch2.7.1 ecrasant silencieusement celle qu'on choisit
REM nous-memes plus loin pour torch 2.10.0/cu130). Sur cu118/cu126/cu128
REM (RTX 10xx-40xx), on garde les deps completes de nano-vllm : la roue
REM qu'il epingle correspond exactement au torch 2.7.1 installe la-bas.
if "%CUDA_VERSION%"=="cpu" (
...
if not "%CUDA_VERSION%"=="cpu" (
    ...
    REM torch epingle une version de Triton PRECISE par version mineure -
    REM confirme par le projet triton-windows lui-meme ^(notes de version^) :
    REM Triton 3.6 exige torch >= 2.10, torch 2.10.0 exige Triton 3.6.0.
    REM Notre ancienne contrainte ^(>=3.0.0,<3.4^) etait calee pour torch
    REM 2.7.1 ^(voir les autres branches GPU^) - completement decalee ici.
    if "%CUDA_VERSION%"=="cu130" (
        uv pip install "triton-windows>=3.6.0,<3.7"
    ) else (
...
REM Ces trois echo (ici et les deux suivants avant/dans la section
REM flash-attn) semblent superflus mais NE PAS LES RETIRER : leur ajout a
REM fait disparaitre, de facon reproductible, un plantage intermittent
REM ("... etait inattendu.", cmd.exe pur, hors PowerShell) survenant a cet
REM endroit precis sur torch 2.10.0/cu130. Cause exacte non confirmee
REM (probable course entre l'affichage de progression de uv et la lecture
REM de la ligne suivante du script), mais le correctif empirique est net
REM et reproductible - confirme sur plusieurs tentatives.
...
REM combinaison verifiee Python 3.11 + PyTorch 2.7 + CUDA 12.8 (RTX 40xx,
REM et RTX 50xx avant le passage a torch 2.10.0/cu130 ci-dessous).
...
echo Detection de l'architecture GPU pour Flash Attention...
if "%CUDA_VERSION%"=="cu128" (
    ...
    echo   Blackwell sm_120 detecte ^(cu130^)
    REM Source : ussoewwin/Flash-Attention-2_for_Windows sur HuggingFace -
    REM build communautaire explicitement marque "unofficial, use at your
    REM own risk" par son propre auteur, le depot officiel Dao-AILab ne
    REM publiant pas encore de roue Windows pour torch 2.10 au moment
    REM d'ecrire ceci. Accepte en connaissance de cause : verifie disponible
    REM pour Python 3.11 specifiquement, contrairement a la variante 3.12
    REM du meme depot, documentee cassee ^(erreurs d'import DLL _fused^) sur
    REM d'autres projets rencontrant la meme combinaison torch2.10+cu130.
    echo Installing Flash Attention 2 ^(build communautaire torch 2.10/cu130^)...
    uv pip install "https://huggingface.co/ussoewwin/Flash-Attention-2_for_Windows/resolve/main/flash_attn-2.8.3+cu130torch2.10.0cxx11abiTRUE-cp311-cp311-win_amd64.whl"
    if errorlevel 1 (
        echo ERROR: Flash Attention failed to install!
        pause
        exit /b 1
...
REM probleme de timing.
echo [9/10] Database migration (Playlists/Workspaces separation)...
echo   Handled automatically at run.bat startup - nothing to do here.
