...
class AceStepConditionGenerationModel(AceStepPreTrainedModel):
    ...
    def generate_audio(
        ...
        **kwargs,
        ...
        prev_vt = None
        velocity_history = []  # for multistep samplers (deis/ipndm)
        ...
        with torch.no_grad():
            for step_idx, (t_curr, t_prev) in enumerate(iterator):
                ...
                # Update x_t based on inference method and sampler
                if infer_method == "sde":
                    ...
                    # Multi-evaluation samplers: midpoint, rk4, bogacki
                    # Build a model_fn that handles CFG, norm clamping, EMA internally
                    ...
                    # Multistep samplers: deis, ipndm — use velocity history
                    ...
                    # Keep only last 3 for memory efficiency
